            ...
            try:
                ...
                for taxial in range(t):
                    ...
                    image_arr_left, image_arr_right = self.slidingWindow_RL(image_data_left[taxial,:,:], 
                        image_data_right[taxial,:,:], threshold_mode = threshold_mode, threshold = threshold)
                    # image_arr_right = self.slidingWindow_RL(image_data_right[taxial,:,:], 
                        # interval = interval, threshold_mode = threshold_mode, threshold = threshold)
                    ...
                    for circlelarge in range(roll):
                        if times >= 1:
                            image_data_pre_left, image_data_pre_right = self.random_interchange_RL(imga_left = image_data_left[taxial,:,:],
                                imga_right = image_data_right[taxial,:,:], imgb_left = image_data_left[random.randint(0, t - 1),:,:], imgb_right = image_data_right[random.randint(0, t - 1),:,:], 
                            ...
                            for circle in range(times - 1):
                                image_data_pre_left, image_data_pre_right = self.random_interchange_RL(imga_left = image_data_pre_left,
                                    imga_right = image_data_pre_right, imgb_left = image_data_left[random.randint(0, t - 1),:,:], 
                                        imgb_right = image_data_right[random.randint(0, t - 1),:,:], size = size , mode = self.pre_augment_mode)
                            image_arr_left, image_arr_right = self.slidingWindow_RL(image_data_pre_left, 
                                    image_data_pre_right, threshold_mode = threshold_mode, threshold = threshold)
                            flage = self.savedata_RL(image_arr_left, image_arr_right, flage)
                ...
                if self.pre_augment_mode == 3:
                    ...
                    image_data_b_right = []
                #original data save
                image_arr_left, image_arr_right  = self.slidingWindow_RL(image_data_left, image_data_right, threshold_mode = threshold_mode, threshold = threshold)
                flage = self.savedata_RL(image_arr_left, image_arr_right, flage)
                
                for circlelarge in range(roll):
                    if times >= 1:
                        image_data_pre_left, image_data_pre_right = self.random_interchange_RL(imga_left = image_data_left,
                            imga_right = image_data_right, imgb_left = image_data_b_left, imgb_right = image_data_b_right, size = size , mode = self.pre_augment_mode) 
                        #repeat agument N-1 times
                        for circle in range(times - 1):
                            image_data_pre_left, image_data_pre_right = self.random_interchange_RL(imga_left = image_data_pre_left,
                                imga_right = image_data_pre_right, imgb_left = image_data_b_left, imgb_right = image_data_b_right, size = size , mode = self.pre_augment_mode)
                        image_arr_left, image_arr_right = self.slidingWindow_RL(image_data_pre_left, 
                                image_data_pre_right, threshold_mode = threshold_mode, threshold = threshold)
                        flage = self.savedata_RL(image_arr_left, image_arr_right, flage)
        return   
