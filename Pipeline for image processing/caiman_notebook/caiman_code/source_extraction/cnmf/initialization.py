...
contain a list of functions to c the neurons and the corresponding traces with
...
def init_neurons_corr_pnr(data, max_number=None, gSiz=15, gSig=None,
    ...
    noise_pixel[noise_pixel<=0.001] =np.inf
...
#    cn[np.isnan(cn)] = 0  # remove abnormal pixels
    ...
    pixel_v = ((ii * 10 + jj) * 1e-8).astype(np.float32)


    all_centers = []
    count=0
    while continue_searching & (count<3):
        tmp_kernel = np.ones(shape=tuple([int(round(gSiz / 2.))] * 2))
        logging.info('gSiz：{0} '.format(gSiz))
        if not isinstance(seed_method, str):
            ...
            count=count+1
            logging.info('search, {0} times.'.format(count))
            # logging.info('tmp_kernel size, {0}.'.format(tmp_kernel.shape[0]))
            logging.info(tmp_kernel)
            ...
            v_max[(v_search != v_max) | (cn < min_corr) | (pnr < min_pnr)] = 0
            v_max[ind_search] = 0
            [rsub_max, csub_max] = v_max.nonzero()  # subscript of seed pixels
            local_max = v_max[rsub_max, csub_max]
            # #raw implementation end

            

            # #guohua implementation
            # logging.info('guohua implementation')
            # v_search = cv2.medianBlur(v_search, 3)
            # v_DOG = cv2.GaussianBlur(v_search, (7, 7), 2)-cv2.GaussianBlur(v_search, (7, 7), 3)
            # v_max = cv2.dilate(v_DOG, tmp_kernel)
            # # automatically select seed pixels as the local maximums
            # v_max[(v_max != v_DOG) | (cn < min_corr) | (pnr < min_pnr)] = 0
            # v_max[ind_search] = 0
            # [rsub_max, csub_max] = v_max.nonzero()  # subscript of seed pixels
            # local_max = v_max[rsub_max, csub_max]

            #guohua implementation end
        ...
        for ith_seed, idx in enumerate(ind_local_max):
            ...
            if (not ind_success) or (np.sum(ai > 0) < min_pixel):
                ...
                noise_box[noise_box<=0.001] =np.inf
    ...
    # data_to_save = {'center': center}
    # name = 'H:/CM2scope_experimental_data/trace_fear_conditioning/train/gzc_rasgrf-ai148d-369/My_V4_Miniscope/Caiman_analysis/center.pkl'
