    ...
    for fff in file_res:
        if fff is not None:
            ...
            for ii in range(np.shape(A)[-1]):
                new_comp = A[:, ii]  # / np.sqrt(A[:, ii].power(2).sum())
                if new_comp.sum() > 0:
                    non_zero_indices = new_comp.nonzero()[0]
                    non_zero_values = new_comp[non_zero_indices].toarray().flatten()

                    a_tot.append(non_zero_values)
                    idx_tot_A.append(idx_[non_zero_indices])
                    idx_ptr_A.append(len(idx_[non_zero_indices]))
                    C_tot[count, :] = C[ii, :]
                    if params.get('init', 'center_psf'):
                        S_tot[count, :] = S[ii, :]
