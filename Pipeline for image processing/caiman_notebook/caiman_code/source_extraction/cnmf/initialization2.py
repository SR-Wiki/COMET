...
def init_neurons_corr_pnr(data, max_number=None, gSiz=15, gSig=None,
                          ...
                          save_video=False, video_name='D:/桌面/idea/8mm LFOV/demos/demos/notebooks/CM2/initialization.mp4',
    ...
    # data_std2 = get_noise_fft(tmp_data.T, noise_method='mean')[0].T
    ...
    # noise_pixel=data_std2
    # pnr = np.divide(data_max, noise_pixel)
    # pnr[pnr < 0] = 0
    # pnr = pnr+np.divide(noise_pixel,np.max(noise_pixel))*np.max(pnr)
    # pnr[pnr < 0] = 0
    ...
    while continue_searching:
        ...
        for ith_seed, idx in enumerate(ind_local_max):
            ...
            if (not ind_success) or (np.sum(ai > 0) < min_pixel):
                ...
                v_search[r2_min:r2_max, c2_min:c2_max] =  pnr_box
                v_search[ind_search] = 0
                # avoid searching nearby pixels
