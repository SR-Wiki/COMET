...
def calculate_psdx_parallel(y_chunk, ind,T):
    dft = cv2.dft(y_chunk.astype(np.float32), flags=cv2.DFT_COMPLEX_OUTPUT).squeeze()[:len(ind)][ind]
    return np.sum(1. / T * dft * dft, 1)

def get_noise_fft_mp(Y, noise_range=[0.25, 0.5], noise_method='logmexp', max_num_samples_fft=3072,
                  opencv=True):
    ...
    if Y.ndim > 1:
        if opencv:
            ...
            try:
                ...
                pass
            psdx_list = []
            Y_reshaped = Y.reshape(-1, T)
            args_list = [(y_chunk, ind,T) for y_chunk in Y_reshaped]
            with Pool() as pool:
                result_chunks = pool.starmap(calculate_psdx_parallel, args_list)
            psdx_list = np.concatenate(result_chunks)
            psdx = np.reshape(psdx_list, Y.shape[:-1] + (-1,))
        else:
            xdft = np.fft.rfft(Y, axis=-1)
            xdft = xdft[..., ind[:xdft.shape[-1]]]
