...
import caiman as cm
from caiman.source_extraction.cnmf.pre_processing import get_noise_fft,get_noise_fft_mp
...
def local_correlations_fft_cuda(Y,
    ...
    if rolling_window is None:
        ...
        num_batches = math.ceil(total_images / batchsize)

        mean_sum = torch.zeros(Y.shape[1:], dtype=torch.float32).to(device)
        square_sum = torch.zeros(Y.shape[1:], dtype=torch.float32).to(device)
        n_samples = 0

        for batch in range(num_batches):
            torch.cuda.empty_cache()
            batch_start = batch * batchsize
            batch_end = min(batch_start + batchsize, total_images)
            batch_data = np.array(Y[batch_start:batch_end]).astype(np.float16)
            Y_batch_tensor = torch.from_numpy(batch_data).to(device)
            batch_mean = torch.mean(Y_batch_tensor, dim=0)
            batch_square_mean = torch.mean(Y_batch_tensor ** 2, dim=0)
            mean_sum += batch_mean.to(dtype=torch.float32) * Y_batch_tensor.shape[0]
            square_sum += batch_square_mean.to(dtype=torch.float32) * Y_batch_tensor.shape[0]
            n_samples += Y_batch_tensor.shape[0]
        
        torch.cuda.empty_cache()
        total_mean = mean_sum / n_samples
        total_variance = (square_sum / n_samples) - (total_mean ** 2)
        total_variance[total_variance == 0]= torch.inf
        Ystd = torch.sqrt(total_variance).cpu().numpy().astype(np.float16)
        Y_mean = total_mean.cpu().numpy().astype(np.float16)

        del Y_batch_tensor, square_sum,mean_sum

        Y -= Y_mean
        # Ystd[Ystd == 0] = np.inf
        Y /= Ystd
        T2 = time.time()
        print('preproceing程序运行时间:%s秒' % ((T2 - T1)))
        # pass
    else:
    ...
    if Y.ndim == 4:
        ...
        if eight_neighbours:
            sz = np.ones((3, 3), dtype='float16')
            sz[1, 1] = 0
        else:
            sz = np.array([[0, 1, 0], [1, 0, 1], [0, 1, 0]], dtype='float16')

    if opencv and Y.ndim == 3:
        # T1 = time.time()
        # YYconv = np.stack([cv2.filter2D(img.astype(np.float32), -1, sz, borderType=0).astype(np.float16)*img for img in Y])

        padding = (3-1 ) // 2
        sz_tensor=torch.from_numpy(sz).float().half()
        device = torch.device("cuda")
        sz_tensor = sz_tensor.view(1, 1, sz.shape[0], sz.shape[1]).to(device)
        ...
        for batch in range(num_batches):
            ...
            batch_data = np.array(Y[batch_start:batch_end]).astype(np.float16)
            # T1 = time.time()
            img_batch_tensor = torch.from_numpy(batch_data).unsqueeze(1).to(device)
            # T2 = time.time()
            # print('cpu-->gpu程序运行时间:%s秒' % ((T2 - T1)))
            # T1 = time.time()
            output_tensors = F.conv2d(img_batch_tensor, sz_tensor, padding=padding)
            output_tensors=output_tensors*img_batch_tensor
            # T2 = time.time()
            # print('gpu程序运行时间:%s秒' % ((T2 - T1)))
            # T1 = time.time()
            Y[batch_start:batch_end,:,:] = output_tensors[:,0,:,:].cpu().numpy()
            # a = output_tensors[:,0,:,:].cpu().numpy()
            # T2 = time.time()
            # print('gpu-->cpu程序运行时间:%s秒' % ((T2 - T1)))
        del output_tensors 
        ...
        MASK = cv2.filter2D(np.ones(Y.shape[1:], dtype='float32'), -1, sz.astype(np.float32), borderType=0).astype(np.float16)
    else:
        Yconv = convolve(Y, sz[np.newaxis, :], mode='constant')
        MASK = convolve(np.ones(Y.shape[1:], dtype='float16'), sz, mode='constant')
...
def save_summary_images_norm(img1,name):
    img=img1.copy()
    min_value = np.nanmin(img)
    img[np.isnan(img)]=min_value
    print("images_minvalue:",min_value)
    max_value = np.max(img)
    print("images_maxvalue:",max_value)
    min_value=0;
    scaled_img = ((img - min_value) / (max_value - min_value) * 65535).astype(np.uint16)
    cv2.imwrite(name, scaled_img)

def save_summary_images(img1,name):
    img=img1.copy()
    min_value = np.nanmin(img)
    img[np.isnan(img)]=0
    img[img < 0] = 0
    ...
    print("images_maxvalue:",max_value)
    min_value=0;
    scaled_img = ((img - 0) / (max_value - 0) * 65535).astype(np.uint16)
    cv2.imwrite(name, scaled_img)


def filter_image(idx, img, psf):
    result = cv2.filter2D(img, -1, psf, borderType=1)
    return idx, result
...
def correlation_pnr(Y, gSig=None, center_psf: bool = True, swap_dim: bool = True,
    ...
    data_filtered_mean1=data_filtered.mean(axis=0)
    if gSig:
        ...
        if center_psf:
            if background_filter == 'box':
                ...
                logging.info('generate psf')
                psf = cv2.getGaussianKernel(ksize[0], gSig[0],
                                            cv2.CV_32F).dot(cv2.getGaussianKernel(ksize[1], gSig[1], cv2.CV_32F).T)
                logging.info('process psf')
                ind_nonzero = psf >= psf[0].max()
                psf -= psf[ind_nonzero].mean()
                psf[~ind_nonzero] = 0
                logging.info('using psf to filter image')
                with multiprocessing.Pool() as pool:
                    args_list = [(idx, img, psf) for idx, img in enumerate(data_filtered)]
                    results = pool.starmap(filter_image, args_list)
                for idx, result in results:
                    data_filtered[idx] = result
                # for idx, img in enumerate(data_filtered):
    ...
    data_std = get_noise_fft_mp(data_filtered.T, noise_method='mean')[0].T
    ...
    tmp_data[tmp_data < 2 * data_std] = 0

    # compute correlation image
    cn = local_correlations_fft(tmp_data, swap_dim=False)

    return cn, pnr,data_max,data_std

def correlation_pnr_cuda(Y, gSig=None, center_psf: bool = True, swap_dim: bool = True,
                    background_filter: str = 'disk') -> Tuple[np.ndarray, np.ndarray]:
    """
    compute the correlation image and the peak-to-noise ratio (PNR) image.
    If gSig is provided, then spatially filtered the video.
    ...
    # filter data
    logging.info('doing data_raw.copy')
    # data_filtered = Y.copy()
    data_filtered = np.array(Y)
    logging.info('computing data_filtered...')
    if gSig:
        if not isinstance(gSig, list):
            gSig = [gSig, gSig]
        ksize = tuple([int(2 * i) * 2 + 1 for i in gSig])
        ...
        if center_psf:
            if background_filter == 'box':
                ...
                psf[~ind_nonzero] = 0
                padding = (ksize[0]-1 ) // 2
                psf=torch.from_numpy(psf).float().half()
                device = torch.device("cuda")
                psf_tensor = psf.view(1, 1, ksize[0], ksize[0]).to(device)

                batchsize=500
                total_images  = data_filtered.shape[0]
                num_batches = math.ceil(total_images / batchsize)
                for batch in range(num_batches):
                    torch.cuda.empty_cache()
                    batch_start = batch * batchsize
                    batch_end = min(batch_start + batchsize, total_images)
                    batch_data = np.array(data_filtered[batch_start:batch_end]).astype(np.float16)
                    img_batch_tensor = torch.from_numpy(batch_data).unsqueeze(1).to(device)
                    output_tensors = F.conv2d(img_batch_tensor, psf_tensor, padding=padding)
                    
                    data_filtered[batch_start:batch_end,:,:] = output_tensors.squeeze().cpu().numpy()
                del output_tensors 
                del img_batch_tensor
                torch.cuda.empty_cache()
    ...
    data_filtered_mean2=data_filtered.mean(axis=0)
    data_filtered -= data_filtered_mean2
    logging.info('computing data_max...')
    data_max = np.max(data_filtered, axis=0)
    logging.info('computing noise...')
    data_std = get_noise_fft_mp(data_filtered.T, noise_method='mean')[0].T
    # data_std=np.std(data_filtered, axis=0)
    # pnr = np.maximum(np.divide(data_max, np.max(data_max)), data_std)
    data_std[data_std<0.001] =np.inf
    pnr = np.divide(data_max, data_std)
    
    # percentile_value = np.percentile(data_max, 99.9)
    # data_std2[data_max >= percentile_value]= percentile_value
    # pnr = pnr+np.divide(data_max,np.max(data_max))*np.max(pnr)

    pnr[pnr < 0] = 0

    # remove small values
    # tmp_data = data_filtered.copy() / data_std
    tmp_data = np.copy(data_filtered)/ data_std
    tmp_data[tmp_data < 3] = 0


    # tmp_data = np.copy(data_filtered)
    # tmp_data = data_filtered
    # tmp_data[tmp_data < 2 * data_std] = 0

    # compute correlation image
    logging.info('computing correlations...')
    cn,std = local_correlations_fft_cuda(tmp_data, swap_dim=False)
    print(cn.dtype)
    del(tmp_data)

    return cn, pnr,data_max,data_std,std

def correlation_pnr_guohua(Y, gSig=None, center_psf: bool = True, swap_dim: bool = True,
                    background_filter: str = 'disk') -> Tuple[np.ndarray, np.ndarray]:
    """
    compute the correlation image and the peak-to-noise ratio (PNR) image.
    If gSig is provided, then spatially filtered the video.
    ...
    # data_filtered_mean2=data_filtered.mean(axis=0)
    # data_filtered -= data_filtered_mean2
    # logging.info('data_filtered')
    # data_max = np.max(data_filtered, axis=0)
    # logging.info('data_max')
    # data_std = get_noise_fft_mp(data_filtered.T, noise_method='mean')[0].T
    # logging.info('data_std')
    # tmp_data = np.copy(data_filtered)
    # tmp_data[tmp_data >  data_std] = 0
    # data_std2 = get_noise_fft_mp(tmp_data.T, noise_method='mean')[0].T
    # logging.info('data_std')
    ...
    # pnr[pnr < 0] = 0
    # pnr = pnr+np.divide(data_std2,np.max(data_std2))*np.max(pnr)
    # pnr[pnr < 0] = 0
    ...
    percentile_value = np.percentile(data_max[data_max > 0], 99.5)
    mask = (data_max > 0) & (data_max > percentile_value)
    data_max[mask] = percentile_value
    pnr = np.divide(data_max, data_std2)
    pnr[pnr < 0] = 0
    pnr = pnr+np.divide(data_max,np.max(data_max))*np.max(pnr)
    pnr[pnr < 0] = 0
    ...
    # # compute correlation image
    # cn = local_correlations_fft(tmp_data, swap_dim=False)
    # logging.info('running_local_correlations_fft')

    tmp_data = np.copy(data_filtered)
    tmp_data[tmp_data < 3 * data_std2] = 0
    # compute correlation image
    cn = local_correlations_fft(tmp_data, swap_dim=False)
    del(tmp_data)

    return cn, pnr,data_max,data_std2
def correlation_pnr_debug(Y, gSig=None, center_psf: bool = True, swap_dim: bool = True,
                    background_filter: str = 'disk') -> Tuple[np.ndarray, np.ndarray]:
    """
    compute the correlation image and the peak-to-noise ratio (PNR) image.
    If gSig is provided, then spatially filtered the video.
    ...
    return cn, data_std,data_std2,data_max,data_filtered_mean1,data_filtered_mean2
