...
def load_memmap(filename: str, mode: str = 'r') -> Tuple[Any, Tuple, int]:
    ...
    Yr = np.memmap(filename, mode=mode, shape=prepare_shape((d1 * d2 * d3, T)), dtype=np.float16, order=order)
    if d3 == 1:
        return (Yr, (d1, d2), T)
    else:
        ...
        for idx, f in enumerate(filenames):
            ...
            if is_3D:
                ...
                if isinstance(f, (str, list)):
                    Yr = cm.load(caiman.paths.fn_relocated(f), fr=1, in_memory=True, var_name_hdf5=var_name_hdf5,outtype=np.float16)
                    print(Yr.dtype)
                else:
                    Yr = cm.movie(f)
            ...
            Yr = np.ascontiguousarray(Yr, dtype=np.float16) + np.float16(0.0001) + np.float16(add_to_movie)
