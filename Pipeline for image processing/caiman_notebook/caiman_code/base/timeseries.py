            ...
            with tifffile.TiffWriter(file_name, bigtiff=bigtiff, imagej=imagej) as tif:
                if "%4d%02d%02d" % tuple(map(int, tifffile.__version__.split('.'))) >= '20200813':
                    ...
                    if compress == 0:
                        ...
                        compressor = 'zlib'
                    tif.write(self, compression=compressor)
                    # tif.write(np.array([foo(i)  for i in range(self.shape[0])],dtype=np.uint8), compression=compressor)
                    # for i in range(self.shape[0]):
                    #     tif.write(foo(i) , compression=compressor)
