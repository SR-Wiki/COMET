...
def hv_view_patches(Yr, A, C, b, f, d1, d2, YrA=None, image_neurons=None, denoised_color=None,
                    cmap='viridis', r_values=None, SNR=None, cnn_preds=None,coors=None):
    """
    Interactive plotting utility for ipython notebook
    ...
    def plot_unit(uid, scl):
        ...
        im_u = (hv.HSV(im_hsv_scl, kdims=['height', 'width'], bounds=(0, 0, d1, d2))
                .opts(aspect='equal', frame_height=500))

        # 添加多边形
        # cc1 = np.clip(coors[uid]['coordinates'][1:-1, 0], 0, d2)
        # cc2 = np.clip(coors[uid]['coordinates'][1:-1, 1], 0, d1)
        cc1 = coors[uid]['coordinates'][1:-1, 0]+0
        cc1_center = np.mean(cc1)
        # print(cc1)
        cc2 = d2-1-coors[uid]['coordinates'][1:-1, 1]
        cc2_center = np.mean(cc2)
        # print(cc2)
        # id_text = hv.Text(cc1_center, cc2_center, str(uid), halign='center', valign='center', fontsize=20).opts(color='yellow')
        polygons = hv.Polygons([{'x': cc1, 'y': cc2}]).opts(fill_alpha=0, color='white', line_color='white', line_width=2)
        # return hv.Layout([im_u*polygons*id_text] + [trace]).cols(1) #im_u + trace
        return hv.Layout([im_u*polygons] + [trace]).cols(1) #im_u + trace
...
def process_data(i, A, thr_method, thr, swap_dim, dims, d2, d1, cm,d):
    pars: Dict = dict()
    ...
    for B in (Bmat if len(dims) == 3 else [Bmat]):
        ...
        if v.shape[0] >= 5 :
            _, (major_axis, minor_axis), _ = cv2.minAreaRect(v[1:-1].astype(np.float32))
            ellipticity = minor_axis / (major_axis+0.001)
        else:
            ellipticity=0
    ...
    area = 0.0
    for t in range(n):
        j = (t + 1) % n  # 循环到下一个顶点,Shoelace公式
        area += v[t][0] * v[j][1]  # x_i * y_{i+1}
        area -= v[j][0] * v[t][1]  # x_{i+1} * y_i
    ...
    pars['neuron_id'] = i + 1
    pars['ellipticity'] = ellipticity
    pars['area']=abs(area) / 2.0
    return pars

def get_contours_mp(A, dims, thr=0.9, thr_method='max', swap_dim=False):
    ...
    with multiprocessing.Pool() as pool:
        # Adjust the range according to the actual value of nr
        results = pool.starmap(process_data, [(i, A, thr_method, thr, swap_dim, dims, d2, d1, cm,d) for i in range(nr)])
        coordinates.extend(results)
    return coordinates
...
def nb_plot_contour(image, A, d1, d2, thr=None, thr_method='max', maxthr=0.2, nrgthr=0.9,
    ...
    for cor in coors:
        ...
        # if cor['ellipticity'] > ellipticity_threshhold and cor['ellipticity'] < 1.0 / ellipticity_threshhold and cor['area'] > 1:
        if cor['area'] > 0.001:
            # 添加满足条件的坐标到cc1和cc2
            cc1.append(np.clip(cor['coordinates'][1:-1, 0], 0, d2))
            cc2.append(np.clip(cor['coordinates'][1:-1, 1], 0, d1))

        # flag=np.sum(np.isnan(cor['coordinates'][1:-1, 0]))>= 1
        # if flag:
        #     coords_x = cor['coordinates'][1:-1, 0]
        #     coords_y = cor['coordinates'][1:-1, 1]

        #     for i in range(len(coords_x)):
        #         if np.isnan(coords_x[i]):
        #             # 如果前面有元素，则处理并添加前一个元素
        #             if i > 0:
        #                 cc1.append(np.clip(coords_x[0:i], 0, d2) - 1000)
        #             # 如果后面有元素，则处理并添加后一个元素
        #             if i < len(coords_x) - 1:
        #                 cc1.append(np.clip(coords_x[i+1:], 0, d2) - 1000)


        #         if np.isnan(coords_y[i]):
        #             if i > 0:
        #                 cc2.append(np.clip(coords_y[0:i], 0, d1) - 1000)
        #             if i < len(coords_y) - 1:
        #                 cc2.append(np.clip(coords_y[i+1:], 0, d1) - 1000)


    # text_labels = [str(i) for i in range(len(coors))]
    p.patches(cc1, cc2, alpha=0.3, color=face_color,
              line_color=line_color, line_width=1, **kwargs)
    # p.text(center[:, 1], center[:, 0], text=text_labels, text_color="yellow", text_align="center", text_baseline="middle",text_font_size="1pt")
    if show:
        bpl.show(p)
    return p
# def remove_NAN(array):
#     rows_with_nan = np.isnan(array)
#     counter_without_nan = array[~rows_with_nan]
#     print(1)
#     return counter_without_nan
...
def nb_inspect_correlation_pnr(corr, pnr,num_bins=200):
    """
    ...
    """
    # 设置颜色图和图像大小
    hv_corr = hv.Image(corr, vdims='corr', label='correlation').opts(cmap='inferno', width=500, height=500)
    hv_pnr = hv.Image(pnr, vdims='pnr', label='pnr').opts(cmap='inferno', width=500, height=500)


    def hist(im, rx, ry,num_bins=num_bins):
        obj = im.select(x=rx, y=ry) if rx and ry else im
        histogram = hv.operation.histogram(obj, num_bins=num_bins)
            # Adjust the opts to set the number of ticks on the axes
    ...
    hist_corr = hv.DynamicMap(
        fct.partial(hist, im=hv_corr), streams=[str_corr]).opts( yticks=list(range(0, 200,5)))
    hist_pnr = hv.DynamicMap(
        fct.partial(hist, im=hv_pnr), streams=[str_pnr]).opts(yticks=list(range(0, 200,5)))
...
def inspect_correlation_pnr(correlation_image_pnr, pnr_image):
    ...
    fig, (ax1, ax2) = pl.subplots(1, 2, figsize=(15, 7), sharex=True, sharey=True)
    pl.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05, wspace=0.1, hspace=0.1)

    im_cn = ax1.imshow(correlation_image_pnr, cmap='inferno')
    ax1.set_title('Correlation Image')
    cbar_cn=pl.colorbar(im_cn, ax=ax1)

    im_pnr = ax2.imshow(pnr_image, cmap='inferno')
    ax2.set_title('PNR')
    cbar_pnr=pl.colorbar(im_pnr, ax=ax2)

    # 调整颜色条的高度
    for cbar in [cbar_cn, cbar_pnr]:
        pos = cbar.ax.get_position()
        cbar.ax.set_position([pos.x0, pos.y0 + pos.height * 0.1, pos.width, pos.height * 0.8])

    # fig = pl.figure(figsize=(10, 4))
    # pl.axes([0.05, 0.2, 0.4, 0.7])
    # im_cn = pl.imshow(correlation_image_pnr, cmap='inferno')
    # pl.title('correlation image')
    # pl.colorbar()
    # pl.axes([0.5, 0.2, 0.4, 0.7])
    # im_pnr = pl.imshow(pnr_image, cmap='inferno')
    # pl.title('PNR')
    # pl.colorbar()
