...
//        const QUrl url(m_cBehavCam["qmlFile"].toString("qrc:/behaviorCam.qml"));
        ...
        QObject::connect(rootObject, SIGNAL(send_scale(double, double, double)),
                             this, SLOT(slot_scale(double, double, double)));
        QObject::connect(rootObject, SIGNAL(send_drag(double, double)),
                             this, SLOT(handle_drag(double, double)));
...
void VideoDevice::slot_scale(double scale, double x, double y)
{
//    if(scale > 0){
//        view->setX(view->x()-x * 0.05);
//        view->setY(view->y()-y * 0.05);
//        view->setWidth(view->width() * 1.05);
//        view->setHeight(view->height() * 1.05);
//    }
//    if(scale < 0){
//        view->setX(view->x()+x- x / 1.05);
//        view->setY(view->y()+y- y / 1.05);
//        view->setWidth(view->width() / 1.05);
//        view->setHeight(view->height() / 1.05);
//    }

    double factor = (scale > 0) ? 1.05 : 0.95;
    double newWidth = view->width() * factor;
    double newHeight = view->height() * factor;

    // 计算新的x和y坐标，使得鼠标位置相对于窗口的位置保持不变
    double newX = x - x * factor;
    double newY = y - y * factor;

    view->setX(newX+view->x());
    view->setY(newY+view->y());
    view->setWidth(newWidth);
    view->setHeight(newHeight);
}

void VideoDevice::handle_drag(double x, double y)
{
    view->setX(view->x()+x);
    view->setY(view->y()+y);
}
...
void VideoDevice::handlePropChangedSignal(QString type, double displayValue, double i2cValue, double i2cValue2)
{
    ...
    //    qDebug() << "Sending updated prop signal to backend";
        ...
        if (type == "led1" || type == "led2"){
            ...
            for (int i = 0; i < displayValue*2+1; i++) {
