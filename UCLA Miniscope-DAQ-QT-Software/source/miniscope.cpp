...
//             }
            last=curr;
            curr=Y;
            if (last != 10000 && curr != 10000) {
                QString str = process(last, curr);
                serial_send(str);
            }
...
QString Miniscope::process(double last, double curr)
{
    double a1 = curr + 2 * M_PI;
    double a2 = curr - 2 * M_PI;
    double pos[] = {curr, a1, a2};

    QVector<double> delta;
    for (double x : pos)
    {
        delta.append(qAbs(x - last));
    }

    double p = pos[qMin(qMin(delta[0], delta[1]), delta[2]) == delta[0] ? 0 : (qMin(delta[1], delta[2]) == delta[1] ? 1 : 2)];

    double turn = (p - last) / (2 * M_PI);

    return QString("{turn : %1}").arg(turn);
}
...
void Miniscope::serial_send(const QString &input)
{
    QSerialPort serial;
    ...
    if(serial.open(QIODevice::ReadWrite)) {
        ...
        qint64 bytesWritten = serial.write(input.toUtf8());
        serial.waitForBytesWritten(1);
        if (bytesWritten == -1) {
            qDebug() << "Failed to write data.";
        } else if (bytesWritten != input.size()) {
            qDebug() << "Not all data was written.";
        } else {
