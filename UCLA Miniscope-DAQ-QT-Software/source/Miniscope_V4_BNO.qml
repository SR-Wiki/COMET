...
Item {
    ...
    signal send_scale(double scale, double x, double y)
    signal send_drag(double x, double x)
...
//        z:0
        ...
        anchors.fill: parent
        hoverEnabled : true
        cursorShape: mouseArea1.containsMouse ? Qt.ClosedHandCursor : Qt.ArrowCursor
//        onPressed: ewl.visible = false
        onReleased:  {lastX=0
                      lastY=0
        }
        onWheel: {      //滚轮
//            var mousePos = mapToItem(root, mouseX, mouseY)
                root.send_scale(wheel.angleDelta.y/120, wheel.x, wheel.y);
        }
        onPositionChanged: {
                    if (pressed) {
                        if(lastX==0 & lastY==0) {
                        lastX=mouseX;lastY=mouseY;
                            ...
                            root.send_drag((mouseX-lastX),(mouseY-lastY));
                            lastX=mouseX; lastY=mouseY;
