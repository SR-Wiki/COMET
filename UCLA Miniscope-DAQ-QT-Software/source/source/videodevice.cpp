...
//        const QUrl url(m_cBehavCam["qmlFile"].toString("qrc:/behaviorCam.qml"));
        ...
        QObject::connect(rootObject, SIGNAL(send_scale(double)),
                             this, SLOT(slot_scale(double)));
...
void VideoDevice::slot_scale(double x)
{
    if(x > 0){
        view->setWidth(view->width() * 1.1);
        view->setHeight(view->height() * 1.1);
    }
    if(x < 0){
        view->setWidth(view->width() * 0.9);
        view->setHeight(view->height() * 0.9);
    }
}
